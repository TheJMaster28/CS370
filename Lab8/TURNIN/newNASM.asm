...
; Lab 8
; April 12, 2019 
; NASM code that takes input for the maxium of the while loop for sum of squares
; then it does a while that computes the sum of squares and outputs the result 
%include "io64.inc"

section .data

_L0:	db "Enter Number:", 0		; global strings for promotes 
_L1:    db "sum of squares of: ",0
_L2:    db " is: ",0
section .text
 	global main
...
main:			;Start of Function 
                ; Do not mess with Function Header
	mov    rbp, rsp		;SPECIAL RSP to RSB for MAIN only
	mov    r8, rsp		;FUNC header RSP has to be at most RBP
	add    r8, -104,		;adjust Stack Pointer for Activation record, need to properly calcuate record
	...
	mov rsi, [rax]		; load rsi with the ident VALUE 
        PRINT_STRING _L1      ; prints string "sum of squares.." then entered number
...
T1:	 			; WHILE TOP target
	...
	mov rbx, rsi		; RHS expresion a number change from 100 to the entered number
